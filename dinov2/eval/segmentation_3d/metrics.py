...
class EMIDECMetrics(BTCVMetrics):

    def __init__(self):
        super().__init__()
        self.post_label = AsDiscrete(to_onehot=5)
        self.post_pred = AsDiscrete(argmax=True, to_onehot=5)


class MSWALMetrics(BTCVMetrics):

    def __init__(self):
        super().__init__()
        self.post_label = AsDiscrete(to_onehot=8)
        self.post_pred = AsDiscrete(argmax=True, to_onehot=8)


def get_metric(dataset_name):
    if dataset_name == "BTCV" or dataset_name == "CT_abdomen_organ_seg" or dataset_name == "MRI_abdomen_seg":
        return BTCVMetrics()
    elif dataset_name == "BraTS":
        return BraTSMetrics()
    elif dataset_name == "LA-SEG":
        return LASEGMetrics()
    elif dataset_name == "TDSC-ABUS" or dataset_name == "CT_abdomen_lesion_seg" or dataset_name == "CT_lung_lesion_seg" or dataset_name == "MRI_ATLAS_seg":
        return LASEGMetrics()  # same as LA-SEG
    elif dataset_name == "MRI_EMIDEC":
        return EMIDECMetrics()
