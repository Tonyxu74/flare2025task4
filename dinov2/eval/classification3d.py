...
def do_finetune(feature_model, autocast_dtype, args):
    ...
    optimizer = torch.optim.AdamW(filter(lambda x: x.requires_grad, cls_model.parameters()), lr=args.learning_rate, weight_decay=0.0)
    ...
    # loss_fn should just be crossentropy
    ...
    for it, train_data in enumerate(train_loader):
        ...
        if it % args.eval_iters == 0:
            ...
            print(f"[Iter {it}], Train loss: {avg_train_loss}, Val bal acc: {val_bal_acc}, Val AUC: {val_auc}, Val loss: {val_loss / val_steps}", flush=True)

            # save best model
            if (val_bal_acc+val_auc)/2 > best_val_metric:
                best_val_metric = (val_bal_acc+val_auc)/2
    ...
    print(f"Test bal acc: {test_bal_acc}, Test AUC: {test_auc}, Test loss: {test_loss / test_steps}")
