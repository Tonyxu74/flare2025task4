...
def get_args():
    ...
    parser.add_argument(
        "--output-dir",
        default="/project/amgrp/txu/flare2025/inference_out/MSWAL",
        type=str,
    ...
    parser.add_argument(
        ...
        default='/project/amgrp/txu/flare2025/CT_MSWAL_seg.json'
    )
    parser.add_argument(
        "--decoder-weights",
        type=str,
        help="Path to decoder weights for segmentation head",
        default='/project/amgrp/txu/flare2025/outputs/it125k_CT_MSWAL_seg_lr1e-3/best_model.pth'
    )
...
dice_metric2 = DiceMetric(include_background=True, reduction="mean_batch", get_not_nans=False)
...
with torch.no_grad():
    for val_data in val_imgs:
        ...
        dice_metric2(y_pred=pred_tensor, y=label_tensor)
        print(val_dice)
        print(dice_metric2.aggregate())
        dice_metric.reset()
        dice_metric2.reset()
