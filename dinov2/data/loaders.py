...
def make_dataset_3d(
    *,
    ...
    #datalist = [x for x in datalist if max(x['shape'][:3]) < 513]
    logger.info(f'3d datalist length: {len(datalist)}')
...
def make_segmentation_dataset_3d(
    ...
    if dataset_name == 'BTCV':
        ...
        datalist_path = os.path.join(base_directory, 'MRI_EMIDEC.json')
        class_num = 5
        input_channels = 1
    elif dataset_name == "CT_abdomen_lesion_seg":
        datalist_path = os.path.join(base_directory, 'CT_abdomen_lesion_seg.json')
        class_num = 2
        input_channels = 1
    elif dataset_name == "CT_abdomen_organ_seg":
        datalist_path = os.path.join(base_directory, 'CT_abdomen_organ_seg.json')
        class_num = 14
        input_channels = 1
    elif dataset_name == "CT_lung_lesion_seg":
        datalist_path = os.path.join(base_directory, 'CT_lung_lesion_seg.json')
        class_num = 2
        input_channels = 1
    elif dataset_name == "MRI_ATLAS_seg":
        datalist_path = os.path.join(base_directory, 'MRI_ATLAS_seg.json')
        class_num = 2
        input_channels = 1
    elif dataset_name == "MRI_abdomen_seg":
        datalist_path = os.path.join(base_directory, 'MRI_abdomen_seg.json')
        class_num = 14
        input_channels = 1
    elif dataset_name == "CT_MSWAL_seg":
        datalist_path = os.path.join(base_directory, 'CT_MSWAL_seg.json')
        class_num = 8
        input_channels = 1
    else:
    ...
    # zip saving error
    if dataset_name == "CT_MSWAL_seg":
        datalist['training'] = [d for d in datalist['training'] if d['image'] != '/project/amgrp/txu/flare2025/CT/val_downstream/MSWAL/imagesTr/MSWAL_0535_0000.nii.gz']
    ...
    if dataset_name == 'CT_MSWAL_seg' or dataset_name == 'MRI_abdomen_seg':
        train_datalist = train_datalist + deepcopy(val_datalist)
        val_datalist = [{**v, 'val': True} for v in val_datalist]
        test_datalist = [{**v, 'val': True} for v in test_datalist]
...
def make_classification_dataset_3d(
    ...
    if dataset_name == 'ICBM':
        ...
        datalist_path = os.path.join(base_directory, 'MRI_ABIDEII.json')
        class_num = 2
    elif dataset_name == 'MRI_UPenn_GBM':
        datalist_path = os.path.join(base_directory, 'MRI_UPenn_GBM.json')
        class_num = None
    elif dataset_name == 'CT_abdomen_disease_classify':
        datalist_path = os.path.join(base_directory, 'CT_abdomen_disease_classify.json')
        class_num = 24
    elif dataset_name == 'MRI_openneuro_phen':
        datalist_path = os.path.join(base_directory, 'MRI_openneuro_phen.json')
        class_num = 4
    elif dataset_name == 'MRI_openneuro_age':
        datalist_path = os.path.join(base_directory, 'MRI_openneuro_age.json')
        class_num = 1
    elif dataset_name == 'MRI_endo_classify':
        if fold == 0:
            datalist_path = os.path.join(base_directory, 'MRI_endo_classify.json')
        elif fold == 1:
            datalist_path = os.path.join(base_directory, 'MRI_endo_classify_fold_2.json')
        elif fold == 2:
            datalist_path = os.path.join(base_directory, 'MRI_endo_classify_fold_3.json')
        class_num = 2
    else:
        raise ValueError(f'Unsupported dataset "{dataset_name}"')
