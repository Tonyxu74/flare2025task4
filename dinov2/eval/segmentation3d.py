...
def do_finetune(feature_model, autocast_dtype, args):
    ...
    if args.segmentation_head == 'UNETR':
        ...
        seg_model = ViTAdapterUNETRHead(feature_model, input_channels, 96, num_classes, autocast_ctx)
    else:
    ...
    if args.train_feature_model:
        ...
        if args.segmentation_head == 'ViTAdapterUNETR':
            if 'lora' not in args.config_file:
                ...
                seg_model.train()
                lora.mark_only_lora_as_trainable(seg_model.feature_model.vit_model)
        else:
            if 'lora' not in args.config_file:
                ...
                for k, v in my_state_dict.items():
                    if 'lora_' in k:
                        ...
                        if args.train_feature_model:
                            ...
                            if args.segmentation_head == 'ViTAdapterUNETR' and 'vit_model' not in k:
                                # for ViTAdapterUNETR, we only save the adapter weights
                                to_save[k] = v
                    else:
                        to_save[k] = v
    ...
    # test
    pr = seg_model.load_state_dict(torch.load(args.output_dir + "/best_model.pth"), strict=False)
    print(pr)
    seg_model.eval()
