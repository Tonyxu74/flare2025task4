...
def train_iter(model, batch, optimizer, scheduler, loss_function, scaler):
    x, y = (batch["image"].cuda(), batch["label"].cuda().float().unsqueeze(1))
    logits = model(x)
...
def do_finetune(feature_model, autocast_dtype, args):
    ...
    # classification head
    cls_model = LinearHead(feature_model, num_classes, autocast_ctx, n_last_layers=args.n_last_layers, cat_tokens=False)
    ...
    optimizer = torch.optim.AdamW(filter(lambda x: x.requires_grad, cls_model.parameters()), lr=args.learning_rate, weight_decay=0.)
    max_iter = args.epochs * args.epoch_length
    scheduler = WarmupCosineSchedule(
        optimizer,
        warmup_steps=args.warmup_iters,
        t_total=max_iter,
        #end_lr=args.learning_rate/3
    )

    # loss_fn should just be mse
        ...
        if it % args.eval_iters == 0:
            ...
            with torch.no_grad():
                for val_data in val_loader:
                    x, y = (val_data["image"].cuda(), val_data["label"].cuda().float().unsqueeze(1))
                    logits = cls_model(x)
            ...
            print(f"[Iter {it}], Train loss: {avg_train_loss}, Val mae: {val_mae}, Val loss: {val_loss / val_steps}", flush=True)
    ...
    with torch.no_grad():
        for test_data in test_loader:
            x, y = (test_data["image"].cuda(), test_data["label"].cuda().float().unsqueeze(1))
            logits = cls_model(x)
            loss = loss_fn(logits, y)
    ...
    test_mae = (np.abs(test_preds - test_labels)).mean()

    print(f"Test mae: {test_mae}, Test loss: {test_loss / test_steps}")
